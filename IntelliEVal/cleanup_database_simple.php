<?php
/**
 * IntelliEVal System - Database Cleanup Script (Simple Version)
    ...
    if ($_POST['action'] === 'run_cleanup') {
        ...
        try {
            ...
            if ($duplicate_categories_count > 0) {
                $delete_duplicate_categories = "DELETE ec1 FROM evaluation_categories ec1 INNER JOIN evaluation_categories ec2 WHERE ec1.name = ec2.name AND ec1.id > ec2.id AND ec1.status = 'active'";
                if (mysqli_query($conn, $delete_duplicate_categories)) {
                    $cleanup_results['duplicate_categories'] = $duplicate_categories_count;
                    $total_cleaned += $duplicate_categories_count;
                } else {
                    $errors[] = "Error cleaning duplicate categories: " . mysqli_error($conn);
                }
            }
...
<head>
    ...
    <title>Database Cleanup - IntelliEVal System</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        .text-seait-dark { color: #1e3a8a; }
        .bg-seait-orange { background-color: #f97316; }
    </style>
</head>
