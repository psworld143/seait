...
$categories_query = "SELECT id, name, evaluation_type FROM main_evaluation_categories WHERE status = 'active' ORDER BY name";
...
// Build query conditions for the selected category
$where_conditions = ["es.main_category_id = ?"]; // Always filter by selected category
...
// Get total count for pagination for selected category
...
// Get evaluations for selected category
...
// Get evaluation statistics for all categories
$stats_query = "SELECT
    ...
    <div class="border-b border-gray-200">
        <nav class="flex space-x-8 px-6" aria-label="Tabs">
            <?php foreach ($categories as $index => $category): ?>
                <a href="?<?php echo http_build_query(array_merge($_GET, ['category' => $category['id'], 'page' => 1])); ?>"
                   class="py-4 px-1 border-b-2 font-medium text-sm <?php echo $selected_category == $category['id'] ? 'border-seait-orange text-seait-orange' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'; ?>">
                    <div class="flex items-center">
                        <i class="fas 
                            <?php
        ...
        <form method="GET" action="" class="space-y-4">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="text-sm text-gray-600">
                    Showing <?php echo number_format($total_records); ?> evaluation(s) in selected category
                </div>
                <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                    ...
                    <a href="?category=<?php echo $selected_category; ?>" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600 transition text-center">
...
<div class="bg-white rounded-lg shadow-md overflow-hidden">
    <div class="px-4 sm:px-6 py-4 border-b border-gray-200">
        <h2 class="text-lg font-medium text-gray-900">
            ...
            echo htmlspecialchars($current_category_name) . ' Evaluations (' . number_format($total_records) . ')';
            ?>
        </h2>
    </div>
    ...
    <?php if (mysqli_num_rows($evaluations_result) == 0): ?>
        <div class="p-6 text-center">
            ...
            <p class="text-gray-500">No evaluations found in this category matching your criteria.</p>
            <?php if ($search || $status_filter || $type_filter || $semester_filter): ?>
                <a href="?category=<?php echo $selected_category; ?>" class="mt-4 inline-block bg-seait-orange text-white px-4 py-2 rounded-lg hover:bg-orange-600 transition">
                    Clear Filters
                </a>
            <?php endif; ?>
        </div>
