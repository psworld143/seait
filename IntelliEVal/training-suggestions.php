...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'generate_suggestions') {
    // Get teachers with average scores less than 4.0 in any evaluation category
    $teachers_query = "SELECT DISTINCT
                      ...
                      ORDER BY u.last_name, u.first_name, average_score ASC";
    ...
    $_SESSION['message'] = "Generated $suggestions_generated training suggestions for $teachers_processed teachers with scores below 4.0.";
    $_SESSION['message_type'] = 'success';
    header('Location: training-suggestions.php');
    exit();
}

// Get training suggestions grouped by teacher
$suggestions_query = "SELECT
                     ...
                     FROM users u
                     JOIN training_suggestions ts ON u.id = ts.user_id
                     WHERE u.role = 'teacher'
                     GROUP BY u.id
                     ORDER BY critical_count DESC, high_count DESC, total_suggestions DESC, u.last_name, u.first_name";
...
// Get teachers with low scores (for display)
...
<div class="mb-6 sm:mb-8">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm sm:text-base text-gray-600">AI-powered training recommendations grouped by teachers (scores below 4.0)</p>
        </div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
    ...
    <div class="training-card">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Teachers with Scores Below 4.0</h3>
        <div class="space-y-3">
            <?php if (mysqli_num_rows($low_scores_result) > 0): ?>
                <?php while($teacher = mysqli_fetch_assoc($low_scores_result)): ?>
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded">
                    <div>
                        <p class="font-medium text-sm"><?php echo htmlspecialchars($teacher['last_name'] . ', ' . $teacher['first_name']); ?></p>
                        <p class="text-xs text-gray-600"><?php echo htmlspecialchars($teacher['sub_category_name']); ?></p>
                    </div>
                    <div class="text-right">
                        <p class="font-medium text-sm <?php echo $teacher['average_score'] < 2.5 ? 'text-red-600' : ($teacher['average_score'] < 3.0 ? 'text-orange-600' : 'text-yellow-600'); ?>">
                            <?php echo round($teacher['average_score'], 2); ?>
                        </p>
                        <p class="text-xs text-gray-500"><?php echo $teacher['total_ratings']; ?> ratings</p>
                    </div>
                </div>
                <?php endwhile; ?>
            <?php else: ?>
                <div class="text-center py-4">
                    ...
                    <p class="text-gray-500 text-sm">All teachers have scores above 4.0!</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-sm">Critical (< 2.5)</span>
                </div>
                <span class="font-medium"><?php echo $stats['critical_suggestions']; ?></span>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <div class="w-4 h-4 bg-orange-500 rounded-full mr-3"></div>
                    <span class="text-sm">High (2.5-3.0)</span>
                </div>
                <span class="font-medium"><?php echo $stats['high_suggestions']; ?></span>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <div class="w-4 h-4 bg-yellow-500 rounded-full mr-3"></div>
                    <span class="text-sm">Medium (3.0-3.5)</span>
                </div>
                <span class="font-medium"><?php echo $stats['medium_suggestions']; ?></span>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <div class="w-4 h-4 bg-green-500 rounded-full mr-3"></div>
                    <span class="text-sm">Low (3.5-4.0)</span>
                </div>
                <span class="font-medium"><?php echo $stats['low_suggestions']; ?></span>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-lg shadow-md p-6">
    <div class="flex justify-between items-center mb-6">
        <h3 class="text-lg font-semibold text-gray-900">Teachers with Training Suggestions</h3>
        <div class="flex space-x-2">
            <a href="export_suggestions.php" class="btn-success btn-sm">
    ...
    <div class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between p-2 bg-gray-50 rounded text-sm">
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-600"><?php echo htmlspecialchars($suggestion['category_name'] ?? 'N/A'); ?></p>
                        </div>
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p class="empty-state-text">No training suggestions found. Click "Generate Suggestions" to create recommendations for teachers with scores below 4.0.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
