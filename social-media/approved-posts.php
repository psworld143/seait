...
/**
 * Get all approved posts with author information
 * @param mysqli $conn Database connection
 * @param int $limit Number of posts to retrieve
 * @return mysqli_result|false Query result
 */
function get_approved_posts($conn, $limit = 50) {
    $limit = (int)$limit;
    ...
    if (!$stmt) {
        error_log("Database error in get_approved_posts (prepare): " . mysqli_error($conn));
        return false;
    }

    mysqli_stmt_bind_param($stmt, "i", $limit);
    if (!mysqli_stmt_execute($stmt)) {
        error_log("Database error in get_approved_posts (execute): " . mysqli_stmt_error($stmt));
    ...
    if (!$result) {
        error_log("Database error in get_approved_posts (get_result): " . mysqli_stmt_error($stmt));
        mysqli_stmt_close($stmt);
        return false;
    }
 ...
 */
function get_approved_post_by_id($conn, $post_id) {
    $post_id = (int)$post_id;
    $query = "SELECT p.*, u.first_name, u.last_name, u.email,
              a.first_name as approver_first_name, a.last_name as approver_last_name
              FROM posts p
...
/**
 * Unapprove an approved post (change back to pending)
 * @param mysqli $conn Database connection
 * @param int $post_id Post ID
 * @param int $unapprover_id User ID who unapproves
 * @return bool Success status
 */
function unapprove_post($conn, $post_id, $unapprover_id) {
    $post_id = (int)$post_id;
    $unapprover_id = (int)$unapprover_id;
...
/**
 * Delete an approved post (soft delete)
 * @param mysqli $conn Database connection
 * @param int $post_id Post ID
 * @return bool Success status
 */
function delete_approved_post($conn, $post_id) {
    $post_id = (int)$post_id;
 ...
 * @param mysqli $conn Database connection
 * @return int Count of approved posts
 */
function get_approved_posts_count($conn) {
    $query = "SELECT COUNT(*) as total FROM posts WHERE status = 'approved'";
 ...
 */
function get_approved_posts_statistics($conn) {
    $stats = [];
    ...
    $avg_query = "SELECT AVG(TIMESTAMPDIFF(MINUTE, created_at, updated_at)) as avg_time
                  FROM posts
                  WHERE status = 'approved' AND approved_by IS NOT NULL";
    $avg_result = mysqli_query($conn, $avg_query);
    $avg_time = mysqli_fetch_assoc($avg_result)['avg_time'];
    $stats['avg_approval_time'] = $avg_time ? round($avg_time, 1) : 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        switch ($_POST['action']) {
            case 'unapprove':
                if (unapprove_post($conn, $post_id, $_SESSION['user_id'])) {
                    $message = "Post moved back to pending successfully!";
                    $message_type = 'success';
                } else {
                    $message = "Error moving post back to pending.";
                    $message_type = 'error';
                }
                break;

            case 'delete':
                if (delete_approved_post($conn, $post_id)) {
...
$approved_posts = get_approved_posts($conn);
$statistics = get_approved_posts_statistics($conn);
$approved_count = get_approved_posts_count($conn);
...
<head>
    ...
    <title>Approved Posts - Social Media Manager - SEAIT</title>
...
<body class="bg-gray-50">
    ...
    <div id="main-content" class="lg:ml-64 pt-20 min-h-screen transition-all duration-300 ease-in-out">
        <div class="p-3 sm:p-4 lg:p-8">
            <div class="mb-6 sm:mb-8">
                <h1 class="text-2xl sm:text-3xl font-bold text-seait-dark mb-2">Approved Posts</h1>
                <p class="text-gray-600">Manage and review approved content</p>
            </div>
                ...
                <div class="bg-white p-4 sm:p-6 rounded-lg shadow-md">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold text-gray-900"><?php echo $statistics['avg_approval_time']; ?>m</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg shadow-md">
                <div class="p-4 sm:p-6 border-b border-gray-200">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
                        <div>
                            <h3 class="text-lg font-semibold text-seait-dark">Approved Content</h3>
                            <p class="text-gray-600">Manage and review approved posts</p>
                        </div>
                ...
                <div class="p-3 sm:p-6">
                    <?php if ($approved_posts && mysqli_num_rows($approved_posts) > 0): ?>
                        <div class="space-y-4 sm:space-y-6">
                            <?php while($post = mysqli_fetch_assoc($approved_posts)): ?>
                            <div class="border border-gray-200 rounded-lg p-4 sm:p-6 hover:shadow-md transition">
                                <div class="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-4">
                                    <div class="flex-1 mb-4 lg:mb-0">
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4 text-sm text-gray-600 mb-3">
                                            ...
                                            <span class="flex items-center">
                                                ...
                                                <span class="truncate">Approved: <?php echo date('M d, Y H:i', strtotime($post['updated_at'])); ?></span>
                                            </span>
                                        </div>
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            <span class="inline-block px-3 py-1 text-xs bg-green-100 text-green-800 rounded">
                                                Approved
                                            </span>
                                            <?php if ($post['approver_first_name']): ?>
                                            <span class="inline-block px-3 py-1 text-xs bg-purple-100 text-purple-800 rounded">
                                                by <?php echo htmlspecialchars($post['approver_first_name'] . ' ' . $post['approver_last_name']); ?>
                                            </span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    <div class="flex flex-wrap gap-2 lg:flex-col lg:gap-2 lg:ml-4">
                                        <button onclick="unapprovePost(<?php echo $post['id']; ?>)"
                                            ...
                                            <i class="fas fa-undo mr-1"></i><span class="hidden sm:inline">Unapprove</span>
                                        </button>
                                ...
                                <div class="text-gray-700 mb-4">
                                    ...
                                    <div class="bg-gray-50 p-4 rounded border-l-4 border-green-500 overflow-hidden">
                    ...
                    <?php elseif ($approved_posts === false): ?>
                        <div class="text-center py-12">
                            <i class="fas fa-exclamation-triangle text-red-500 text-6xl mb-4"></i>
                            <h3 class="text-xl font-semibold text-gray-700 mb-2">Database Error</h3>
                            <p class="text-gray-600">Unable to load approved posts. Please try again later.</p>
                        </div>
                    <?php else: ?>
                        <div class="text-center py-12">
                            <i class="fas fa-inbox text-gray-400 text-6xl mb-4"></i>
                            <h3 class="text-xl font-semibold text-gray-700 mb-2">No Approved Posts</h3>
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full animate-bounce-in">
                <div class="p-6 text-center">
                    ...
                    <form id="unapproveForm" method="POST" class="space-y-3">
                        <input type="hidden" name="action" value="unapprove">
                        <input type="hidden" name="post_id" id="unapprovePostId">
            ...
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full animate-bounce-in">
                <div class="p-6 text-center">
                    <div class="mb-4">
                        ...
                        <h3 class="text-xl font-semibold text-gray-900 mb-2">Delete Approved Post</h3>
                        <p class="text-gray-600 mb-4">Are you sure you want to delete this approved post? This action cannot be undone.</p>
    ...
    <script>
        ...
        function unapprovePost(postId) {
            document.getElementById('unapprovePostId').value = postId;
            document.getElementById('unapproveModal').classList.remove('hidden');
        }

        function closeUnapproveModal() {
            document.getElementById('unapproveModal').classList.add('hidden');
        }
