...
// Debug: Check session and database connection
error_log("Profile Debug - Session started");
error_log("Profile Debug - User ID: " . ($_SESSION['user_id'] ?? 'NOT SET'));
error_log("Profile Debug - User Role: " . ($_SESSION['role'] ?? 'NOT SET'));

// Check if user is logged in and is a social media manager
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'social_media_manager') {
    error_log("Profile Debug - User not logged in or wrong role, redirecting to login");
    header('Location: ../login.php');
    exit();
}

error_log("Profile Debug - User authenticated successfully");
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_profile') {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                $update_query = "UPDATE users SET first_name = ?, last_name = ?, email = ?, phone = ?, bio = ? WHERE id = ?";
                $stmt = mysqli_prepare($conn, $update_query);
                mysqli_stmt_bind_param($stmt, "sssssi", $first_name, $last_name, $email, $phone, $bio, $_SESSION['user_id']);
        ...
        if (password_verify($current_password, $user['password'])) {
            ...
            if (isset($_FILES['profile_photo']) && $_FILES['profile_photo']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!in_array($file['type'], $allowed_types)) {
                    ...
                    error_log("Profile Debug - Current working directory: " . getcwd());
                    error_log("Profile Debug - Script directory: " . dirname(__FILE__));
                    
                    // Use absolute path for better reliability
                    $upload_dir = dirname(__FILE__) . '/../uploads/profile-photos/';
                    error_log("Profile Debug - Upload directory path: " . $upload_dir);
                    
                    // Ensure directory exists with proper permissions
                    if (!is_dir($upload_dir)) {
                        if (!mkdir($upload_dir, 0755, true)) {
                            $error = 'Failed to create upload directory. Please check permissions.';
                            error_log("Profile Debug - Failed to create directory: " . $upload_dir);
                        }
                    }
                ...
                if (move_uploaded_file($file['tmp_name'], $filepath)) {
                    ...
                    if (mysqli_stmt_execute($stmt)) {
                        ...
                        error_log("Profile Debug - Photo uploaded and database updated successfully");
                    } else {
                        $error = 'Failed to update profile photo in database.';
                        error_log("Profile Debug - Database update failed: " . mysqli_error($conn));
                    }
                } else {
                    $error = 'Failed to upload profile photo.';
                    error_log("Profile Debug - File upload failed. Upload error: " . $file['error']);
                    error_log("Profile Debug - Source: " . $file['tmp_name'] . " -> Destination: " . $filepath);
                    
                    // Check if destination directory exists and is writable
                    if (!is_dir($upload_dir)) {
                        error_log("Profile Debug - Upload directory does not exist: " . $upload_dir);
                    } elseif (!is_writable($upload_dir)) {
                        error_log("Profile Debug - Upload directory not writable: " . $upload_dir);
                    }
                }
            }
        } else {
...
error_log("Profile Debug - User data fetched: " . ($user ? 'SUCCESS' : 'FAILED'));
...
$stats_query = "SELECT
    ...
    SUM(CASE WHEN type = 'carousel' THEN 1 ELSE 0 END) as carousel_posts
...
error_log("Profile Debug - Stats fetched: " . ($stats ? 'SUCCESS' : 'FAILED'));
...
error_log("Profile Debug - Activity fetched: " . ($activity_result ? 'SUCCESS' : 'FAILED'));
?>
...
?>
        <div class="p-3 sm:p-4 lg:p-8">
            <div class="mb-8">
                ...
                <p class="text-gray-600">Manage your account information, photo, and social media settings</p>
            </div>
                ...
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="text-sm text-blue-700 space-y-1">
                            <p><strong>Account Information:</strong> Update your personal information including name, email, phone, and bio.</p>
                            <p><strong>Profile Photo:</strong> Upload and manage your profile picture with real-time preview.</p>
                            <p><strong>Social Media Statistics:</strong> Track your content management activity and performance metrics.</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="space-y-6">
                    <!-- Account Statistics -->
                    <div class="bg-white rounded-lg shadow-md p-6">
                        <h3 class="text-lg font-semibold text-seait-dark mb-4">Social Media Statistics</h3>
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between">
                                <span class="text-gray-600">Carousel Posts</span>
                                <span class="font-semibold text-purple-600"><?php echo $stats['carousel_posts']; ?></span>
                            </div>
                        </div>
                    </div>
