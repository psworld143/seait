<?php
/**
 * Universal Error Handler for SEAIT System
 * Include this file at the top of all user pages after session_start()
 */
...
// Set error handler to redirect to 505.php for server errors
function customErrorHandler($errno, $errstr, $errfile, $errline) {
    ...
    // Check if it's a fatal error or server-related error
    if (in_array($errno, [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR])) {
        // Redirect to 505.php for server errors
        header("Location: /seait/505.php");
        exit();
    } else {
        // Redirect to 404.php for other errors
...
function checkDatabaseQuery($result, $query_name = "Unknown") {
    if (!$result) {
        ...
        error_log("Database error in $query_name query: $error_message");
        if (!headers_sent()) {
            header("Location: /seait/505.php");
            exit();
        } else {
...
function checkDatabaseStatement($stmt, $query_name = "Unknown") {
    if (!mysqli_stmt_execute($stmt)) {
        global $conn;
        $error_message = mysqli_error($conn);
        error_log("Database error in $query_name statement: $error_message");
        if (!headers_sent()) {
            header("Location: /seait/505.php");
            exit();
        } else {
...
function logErrorWithContext($error_message, $context = []) {
    ...
    error_log("Error with context: " . json_encode($log_data));
}
