<?php
/**
 * Improved Arduino Communicator for SEENS RFID System
 * Better handling of card detection timing and communication
 */

class ArduinoCommunicatorImproved {
    private $port;
    private $handle = null;
    ...
    public function connect() {
        try {
            ...
            // More flexible connection check - accept any response from Arduino
            $this->isConnected = ($response && strlen($response) > 0);
            
            if ($this->isConnected) {
                error_log("Arduino Communicator: Successfully connected to {$this->port}");
                error_log("Arduino Communicator: Response was: " . $response);
            } else {
                error_log("Arduino Communicator: Arduino not responding on {$this->port}");
                error_log("Arduino Communicator: Response was: " . ($response ? $response : 'NO RESPONSE'));
            }
    ...
    /**
     * Read response from Arduino with better timing
     */
    private function readResponse($timeout = 5) {
        ...
        while ((time() - $startTime) < $timeout) {
            ...
            usleep(50000); // 50ms delay for better responsiveness
        }
    ...
    /**
     * Write data to RFID card with improved timing
     */
    public function writeToCard($data, $block = 4) {
        ...
        $response = $this->readResponse(15); // Longer timeout for card operations
    ...
    /**
     * Read data from RFID card with improved timing
     */
    ...
    private function clearBuffer() {
        if ($this->handle) {
            ...
            while (fread($this->handle, 128) !== false) {
    ...
    /**
     * Wait for card detection (useful for debugging)
     */
    public function waitForCard($timeout = 10) {
        if (!$this->isConnected) {
            return false;
        }
        ...
        while ((time() - $startTime) < $timeout) {
            $data = fread($this->handle, 128);
            if ($data !== false && $data !== '') {
                if (strpos($data, 'CARD_DETECTED') !== false) {
                    return true;
                }
            }
            usleep(100000); // 100ms
