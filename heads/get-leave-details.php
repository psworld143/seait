...
if (mysqli_num_rows($result) === 0) {
    echo '<div class="text-center text-red-600">Leave request not found or not in your department.</div>';
    exit();
}
    ...
    <?php if ($leave['status'] === 'pending'): ?>
        <div class="bg-white rounded-lg p-4 border border-gray-200">
            ...
            <div class="flex space-x-3">
                <button onclick="approveLeave(<?php echo $leave['id']; ?>)" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors duration-200">
                    <i class="fas fa-check mr-2"></i>Approve
                </button>
                <button onclick="rejectLeave(<?php echo $leave['id']; ?>)" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors duration-200">
